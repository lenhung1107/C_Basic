...
void InsertNode(elmType x,tree_t *root ){ 
  if (*root == NULL){/* Create a new node for element x */ 
    // printf("insert %d\n", x);
    *root= create_node(x);
  }
  else if (compare(x,(*root)->element) < 0) InsertNode(x, &((*root)->left));
  else if (compare(x,(*root)->element) > 0) InsertNode(x, &((*root)->right)); 
}
...
void InsertandCount(elmType x, tree_t *root) {
  ...
  else if (compare(x,(*root)->element) < 0) InsertandCount(x, &((*root)->left));
  else if (compare(x,(*root)->element) > 0) InsertandCount(x, &((*root)->right));
