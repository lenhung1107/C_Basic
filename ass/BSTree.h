...
treeType search(char* x, treeType root){
  ...
  else if(compare(root->element.Ma, x) == 0){//Neu tim thay
    return root;
  }
  else if(compare(root->element.Ma, x) < 0){//Neu < khoa can tim -> tiep tuc tim ben phai
...
void insertNode(elementType x, treeType *root){    //Neu truyen vao con tro -> thay doi ca tree. Neu chi truyen vao root bthg -> phai return de lay gia tri
  ...
  else if(compare(x.Ma, (*root)->element.Ma) < 0)//Neu khoa them vao < khoa nut goc 
    insertNode(x, &(*root)->left);
  
  else if(compare(x.Ma, (*root)->element.Ma) > 0)
...
void inOrderPrint(treeType root){
  if(root != NULL){
    ...
    // printf("%-20s%-20s%-10.2lf\n", root->element.Ma, root->element.password, root->element.diem);
...
void deleteNode(char* x, treeType *root){
  if(*root != NULL){
    if(compare(x, (*root)->element.Ma) < 0){
      deleteNode(x, &(*root)->left);
    }
    else if(compare(x, (*root)->element.Ma) > 0){
